<x-app-layout>
    <div class="container mx-auto mt-5">
        <div class="mb-4">
            <h2 class="text-2xl font-bold text-gray-800">Surat Pernyataan Ganti Rugi (SPGR) Tanah</h2>
        </div>
        <div class="flex justify-between mb-4">
            @if (Auth::user()->role == 'Admin')
                <a class="btn bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded"
                    href="{{ route('spgr.create') }}">+ Tambah Surat Baru</a>
                <form action="{{ route('spgr.index') }}" method="GET" class="flex">
        ...
        <div class="bg-white rounded-lg shadow-md">
            <table class="w-full table-auto">
                <thead class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    <tr>
                        @foreach (['No', 'Tanggal', 'No.Surat', 'No.Registrasi', 'Penjual', 'Pembeli', 'Detail', 'Letak Surat'] as $header)
                ...
                <tbody class="text-gray-600 text-sm">
                    @if ($spgr->isEmpty())
                        <tr>
                            <td colspan="9" class="text-center py-3 px-2">Data tidak ditemukan.</td>
                        </tr>
                    @else
                        @foreach ($spgr as $item)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-2 text-center">{{ $item->penjual }}</td>
                                <td class="py-3 px-2 text-center">{{ $item->pembeli }}</td>
                                <td class="py-3 px-2 text-center">
                                    <button class="btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded "
                                        ...
                                        <i class="fas fa-info-circle"></i> Detail SPGR
                                    </button>
                                </td>
                                ...
                                @if (Auth::user()->role == 'Admin')
                                    <td class="py-3 px-2">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('spgr.edit', $item->id) }}"
                                                class="text-blue-600 hover:text-blue-800">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('spgr.destroy', $item->id) }}" method="POST"
            ...
            <div class="bg-white rounded-lg shadow-lg w-3/4 md:w-1/2 relative">
                ...
                <div class="p-4">
                    <table class="w-full border border-black">
                        <tbody>
                            <tr class="border">
                                <th class="text-left py-2 px-2 border-r w-1/4">Kelurahan</th>
                                <td id="modalKelurahan" class="border-b p-2"></td>
                            </tr>
                            <tr class="border">
                                <th class="text-left py-2 px-2 border-r">RT / RW</th>
                                <td id="modalRT" class="border-b p-2"></td>
                            </tr>
                            <tr class="border">
                                <th class="text-left py-2 px-2 border-r">Batas – Batas Sempadan Tanah</th>
                                <td id="modalBatas" class="border-b">
                                    <table class="w-full border-collapse">
                                        <tbody>
                                            <tr>
                                                <td class="border-r p-2 w-1/4">Utara: <span id="modalUtara"></span></td>
                                                <td class="border-r p-2 w-1/4">Selatan: <span id="modalSelatan"></span>
                                                </td>
                                                <td class="border-r p-2 w-1/4">Barat: <span id="modalBarat"></span></td>
                                                <td class="p-2 w-1/4">Timur: <span id="modalTimur"></span></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
                            ...
                            <tr class="border">
                                <th class="text-left py-2 px-2 border-r">Ukuran Tanah</th>
                                <td id="modalUkuran" class="border-b"></td>
                            </tr>
                            <tr class="border">
                                <th class="text-left py-2 px-2 border-r">Luas Tanah</th>
                                <td id="modalLuasTanah" class="border-b p-2"></td>
                            </tr>
                            <tr class="border">
                                <th class="text-left py-2 px-2 border-r">Dasar Surat Tanah</th>
                                <td id="modalDasarSurat" class="border-b p-2"></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
        ...
        <script>
            ...
            <table class="w-full border-collapse">
                <tbody>
                    <tr>
                        <td class="border-r p-2 w-1/4">${ukuranUtara}</td>
                        <td class="border-r p-2 w-1/4">${ukuranSelatan}</td>
                        <td class="border-r p-2 w-1/4">${ukuranTimur}</td>
                        <td class="p-2 w-1/4">${ukuranBarat}</td>
                    </tr>
                </tbody>
            </table>
        `;
