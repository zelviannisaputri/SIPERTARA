<x-app-layout>
    <div class="container mx-auto mt-5">
        <div class="mb-4">
            <h2 class="text-2xl font-bold text-gray-800">Surat Keterangan Pendaftaran Tanah (SKPT)</h2>
        </div>
        <div class="flex justify-between mb-4">
            @if (Auth::user()->role == 'Admin')
                <a class="btn bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded"
                    href="{{ route('skpt.create') }}">+ Tambah Surat Baru</a>
                <form action="{{ route('skpt.index') }}" method="GET" class="flex">
        ...
        <div class="bg-white rounded-lg shadow-md">
            <table class="w-full table-auto">
                <thead class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                    <tr>
                        @foreach (['No', 'Tanggal', 'No.Surat', 'No.Registrasi', 'Nama Pemilik', 'Detail', 'Letak Surat'] as $header)
                    ...
                    @else
                        @foreach ($skpt as $item)
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td class="py-3 px-2 text-center">{{ $item->nama }}</td>
                                <td class="py-3 px-2 text-center">
                                    <button class="btn bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded "
                                        ...
                                        <i class="fas fa-info-circle"></i> Detail SKPT
                                    </button>
                                </td>
                                ...
                                @if (Auth::user()->role == 'Admin')
                                    <td class="py-3 px-2">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('skpt.edit', $item->id) }}"
                                                class="text-blue-600 hover:text-blue-800">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('skpt.destroy', $item->id) }}" method="POST"
